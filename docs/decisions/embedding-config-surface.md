...
> **일부 대체됨.** 이제 백엔드가 존재하고 검색은 hybrid다 —
> [[hybrid-retrieval]] 참고. 아래의 설정 표면은 바뀌지 않고 유지됐고, 그것이
> 핵심이었다 — 키 하나(`api`, 그리고 환경변수 전용 API 키)만 추가됐고 이름이
> 바뀐 것은 없다. 크기 측정과 원격 엔드포인트 정책은 적힌 그대로 유효하다.
